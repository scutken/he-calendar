...
- ✅ **节日倒数列表**：新增节日倒数面板，可查看未来365天所有节日，按月分组显示
- ✅ **节日详情卡片**：点击节日标签弹出详情卡片，显示简介、倒数天数，一键跳转百科
- ✅ **节日简介数据**：为农历传统节日、公历现代节日、国际节日、二十四节气添加详细简介
- ✅ **连休天数显示**：法定假日显示连续休假天数（如"休3天"）

#### 交互优化

- 🚀 **节日名称标准化**：合并相似节日名称（如"清明"与"清明节"），避免重复显示
- 🚀 **卡片交互优化**：全局点击监听，点击卡片外区域自动关闭
...
- 🚀 **底层库迁移**：从 lunar-javascript 迁移至 tyme4ts（6tail），更优的设计、更强的扩展性、更准确的节气计算

#### 节日系统重构

- ✅ **节日四大分类**：农历传统节日、公历现代节日、二十四节气、国际节日
- ✅ **节日显示优先级**：农历传统节日 > 公历现代节日 > 节气 > 国际节日 > 农历日期

#### UI/交互优化

- 🎨 **节日配置折叠面板**：默认折叠显示已选数量，点击展开
- 🎨 **标签式开关**：节日开关改为紧凑标签按钮，点击切换选中状态
- 🎨 **黄历详情节日展示**：右侧黄历面板新增节日标签显示区域，不同类型节日显示不同颜色
- ✅ **百科跳转**：点击节日标签可跳转至百度百科对应词条

---

### v1.0.3 (2026-02-04)

- ✅ **设置持久化**：修复 uTools 环境下设置（颜色模式、主题、周起始日）重启后丢失的问题，改用 `utools.dbStorage` 持久化 API。
- 🐞 **外部链接跳转修复**：修复 uTools 环境下 GitHub 源码和网页版地址无法跳转的问题，改用 `utools.shellOpenExternal` API 打开外部链接。
